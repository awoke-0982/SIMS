...
$search_query = mysqli_query($conn, "SELECT * FROM student_info WHERE REG_NO = '$regNo' and STUDENT_ID != '$id' ");
		$num_row = mysqli_num_rows($search_query);
		if($num_row >= 1){
			echo '<script>alert("RegNo is not available."); location.replace(document.referrer);</script>';
