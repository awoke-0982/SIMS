...
success:function(data)
{
  ...
  <table id="students" class="table table-hover table-bordered">
    ...
    <?php
    include 'db.php';
    $sql=  mysqli_query($conn, "SELECT * FROM student_info WHERE USER_TYPE not in('STUDENT')");
    ...
    <form method="post" action="update_subject.php">
      <tr>
        ...
        <td><a data-toggle="modal" data-target="#edit_user" data-id="<?php echo $row['STUDENT_ID'] ?>" id="getUser"><i class="fa fa-pencil-square" aria-hidden="true"></i> edit</a></td>
      </tr>
      </form>
      <?php
    } mysqli_close($conn);
