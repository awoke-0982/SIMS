...
success:function(data)
{
  ...
  ?>
          ...
          <h1 class="page-header">STUDENT <button style="float:right" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal">
  ...
  <table id="students" class="table table-bordered table-condensed">
    <thead>
      <tr id="heads">
        ...
        <th style="width:20%;text-align:center">Program Enrolled</th>
    ...
    <?php
    include 'db.php';
    $sql=  mysqli_query($conn, "SELECT * FROM student_info order by FIRSTNAME ");
    while($row = mysqli_fetch_assoc($sql)) {
      $sid = $row['STUDENT_ID'];
